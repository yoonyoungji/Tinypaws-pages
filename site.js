...
const copy = {
  ko: {
    common: { home: "홈", support: "지원", privacy: "개인정보처리방침", releases: "버전 기록", download: "TinyPaws 다운로드", latest: "최신 버전 1.6.10", email: "문의 이메일", developer: "개발자: youngji Yoon", response: "답변은 보통 3영업일 안에 드립니다.", footer: "TinyPaws 공식 지원 및 개인정보처리방침", close: "닫기" },
    index: {
      ...
      companions: "열 명의 데스크톱 친구", companionsLead: "메뉴 막대의 여덟 개 항목에서 고양이 두 친구와 비밀스러운 두 친구를 포함한 열 명의 캐릭터를 만날 수 있어요.",
      features: "내 화면에 맞게 설정", featureItems: [["1·3·5마리 함께", "메뉴 막대에서 캐릭터 수를 고르면 각 친구가 서로 다른 위치와 방향으로 움직여요."], ["걷기 스타일", "화면 안을 자유롭게 걷거나, 모니터 테두리를 따라 걷게 할 수 있어요."], ["이동 범위", "현재 모니터 안에서만 또는 모든 연결된 모니터에서 움직이게 설정하세요."], ["디테일 조절", "사이즈, 투명도, 걷기 속도를 바꿔 작업 흐름에 맞출 수 있어요."]],
      ...
      release: "1.6.10 업데이트", releaseLead: "더 가벼워진 움직임과 마법소녀의 새로운 마법 이펙트를 만나보세요.",
    ...
    support: {
      title: "TinyPaws 지원", eyebrow: "TinyPaws 지원", heading: "작은 Mac 친구를 위한 도움말.", lead: "TinyPaws 사용 중 도움이 필요하거나, 궁금한 점·오류·기능 제안이 있다면 언제든지 연락해 주세요.", overview: "앱 한눈에 보기", overviewItems: ["macOS 화면 위를 걷는 애니메이션 데스크톱 펫", "여덟 개 항목에서 열 명의 캐릭터 선택", "메뉴 막대에서 1·3·5마리 동시 표시", "사이즈·투명도·걷기 속도 조절", "자유 걷기 또는 테두리 따라 걷기", "현재 모니터 또는 전체 모니터 이동 범위 설정"], permissions: "권한 안내", permissionsLead: "키보드 반응 기능을 사용하려면 macOS의 손쉬운 사용 및 입력 모니터링 권한이 필요할 수 있습니다. 앱 메뉴의 ‘권한 안내 보기’에서 설정 위치를 확인할 수 있어요.", compatibility: "호환성", compatibilityItems: ["플랫폼: macOS", "최소 버전: macOS 13.0 이상", "형태: 메뉴 막대 데스크톱 컴패니언"], faq: "자주 묻는 질문", faqItems: [["캐릭터가 키보드 입력에 반응하지 않아요.", "시스템 설정 > 개인정보 보호 및 보안에서 TinyPaws의 손쉬운 사용 및 입력 모니터링 권한을 확인한 뒤 앱을 다시 열어 주세요."], ["캐릭터 수는 어떻게 바꾸나요?", "메뉴 막대의 ‘캐릭터 수’에서 1마리, 3마리 또는 5마리를 선택하세요."], ["캐릭터가 화면 일부를 가려요.", "캐릭터를 드래그해 옮기거나 메뉴 막대에서 사이즈와 투명도를 조절해 보세요."], ["캐릭터는 어떻게 바꾸나요?", "메뉴 막대의 캐릭터 선택 메뉴에서 원하는 친구를 고를 수 있어요."], ["오류를 제보하거나 기능을 제안하고 싶어요.", "macOS 버전, TinyPaws 버전, 문제를 재현하는 방법을 함께 적어 문의 이메일로 보내 주세요."]]
    },
    privacy: {
      title: "TinyPaws 개인정보처리방침", eyebrow: "TinyPaws 개인정보처리방침", heading: "TinyPaws 개인정보처리방침", lead: "이 방침은 TinyPaws가 사용하는 정보, 기기에 저장되는 정보, 그리고 앱이 수집하지 않는 정보를 설명합니다.", effective: "시행일: 2026년 7월 28일", local: "1. 기기에 저장되는 정보", localLead: "TinyPaws는 다음 실행 때 설정을 복원하기 위해 소량의 앱 설정을 Mac에 저장합니다.", localItems: ["선택한 캐릭터", "선택한 사이즈", "선택한 투명도", "선택한 걷기 속도", "선택한 이동 범위와 걷기 스타일", "무작위로 생성된 설치 식별자"], localNote: "이 설정은 표준 macOS 앱 저장소를 통해 기기에 저장됩니다. 설치 식별자는 아래의 익명 업데이트 통계에만 사용됩니다.", permissions: "2. 권한", permissionsLead: "TinyPaws의 핵심 기능에는 온라인 계정이나 클라우드 서비스가 필요하지 않습니다. 키보드 반응 기능을 쓰는 경우에만 macOS 권한이 필요할 수 있으며, 해당 권한은 기기에서만 처리됩니다.", logs: "3. 진단 로그", logsLead: "앱 상태와 문제 해결을 위해 Mac의 임시 로그 파일에 간단한 진단 메시지를 기록할 수 있습니다.", logsNote: "로그는 사용자가 지원팀에 직접 공유하지 않는 한 기기에 남아 있습니다.", analytics: "4. 익명 업데이트 통계", analyticsLead: "새 버전을 설치하거나 업데이트한 뒤 처음 실행할 때, TinyPaws는 업데이트 사용자를 집계하기 위해 무작위 설치 식별자와 앱 버전만 안전한 연결로 전송할 수 있습니다. 이름, 이메일, IP 주소, 기기 정보, 사용 행동은 전송하거나 저장하지 않습니다. 이 식별자는 앱을 삭제하면 함께 사라집니다.", none: "5. 수집하지 않는 정보", noneItems: ["계정 등록 정보", "결제 정보", "개인 식별 기반 분석 프로필", "클라우드 동기화 데이터", "브라우징 기록", "일반 사용 중 개발자에게 전송되는 개인정보"], third: "6. 제3자 서비스", thirdLead: "업데이트 사용자 수 집계에는 Cloudflare의 서버 인프라를 사용할 수 있습니다. 이 서비스는 앱의 핵심 데스크톱 컴패니언 기능에 필요하지 않습니다.", choices: "7. 사용자의 선택", choicesItems: ["언제든 이 기기에서 TinyPaws 설정을 검토하고 변경할 수 있습니다.", "언제든 앱을 삭제할 수 있습니다.", "앱 작동 방식에 관한 질문은 지원팀에 문의할 수 있습니다."], changes: "8. 방침 변경", changesLead: "TinyPaws의 기능 또는 데이터 처리 방식이 바뀌면 이 방침을 업데이트할 수 있습니다. 중요한 변경 사항은 개정 시행일과 함께 이 페이지에 반영합니다."
    }
  },
  en: {
    common: { home: "Home", support: "Support", privacy: "Privacy Policy", releases: "Release history", download: "Download TinyPaws", latest: "Latest version 1.6.10", email: "Support email", developer: "Developer: youngji Yoon", response: "Typical response time: within 3 business days", footer: "TinyPaws official support and privacy information", close: "Close" },
    index: {
      ...
      companions: "Ten desktop companions", companionsLead: "Meet ten characters across the same eight choices shown in the menu bar, including two cats and two secret friends.",
      features: "Made to fit your desktop", featureItems: [["Choose 1, 3, or 5", "Pick a character count from the menu bar and each friend moves from a different place and direction."], ["Walking style", "Let your companion wander freely or follow the edge of a display."], ["Movement range", "Keep it on the current display or let it explore every connected monitor."], ["Fine-tune the details", "Adjust size, opacity, and walk speed for the way you work."]],
      ...
      release: "New in 1.6.10", releaseLead: "Enjoy lighter movement and Magic Girl's new charged spell effect.",
    ...
    support: {
      title: "TinyPaws Support", eyebrow: "TinyPaws Support", heading: "Help for your tiny Mac companion.", lead: "Need help using TinyPaws, have a question, found a bug, or want to suggest a feature? We would love to hear from you.", overview: "App overview", overviewItems: ["An animated desktop pet for macOS", "Ten characters across eight menu choices", "Show 1, 3, or 5 companions from the menu bar", "Size, opacity, and walk-speed controls", "Free wandering or edge-following movement", "Movement range for the current display or all displays"], permissions: "Permissions", permissionsLead: "Keyboard-reactive features may need macOS Accessibility and Input Monitoring permission. Use “View Permission Guide” in the app menu to find the right settings.", compatibility: "Compatibility", compatibilityItems: ["Platform: macOS", "Minimum version: macOS 13.0 or later", "App type: menu bar desktop companion"], faq: "Frequently asked questions", faqItems: [["My character does not react to keyboard input.", "Check TinyPaws in Accessibility and Input Monitoring under System Settings > Privacy & Security, then reopen the app."], ["How do I change the number of characters?", "Choose 1, 3, or 5 from ‘Number of Characters’ in the menu bar."], ["The character covers part of my screen.", "Drag it somewhere else, or use the menu bar controls to adjust size and opacity."], ["How do I change characters?", "Choose the friend you want from the character picker in the menu bar."], ["I want to report a bug or request a feature.", "Please include your macOS version, TinyPaws version, and steps to reproduce the issue in your email."]]
    },
    privacy: {
      title: "TinyPaws Privacy Policy", eyebrow: "TinyPaws Privacy Policy", heading: "Privacy Policy for TinyPaws", lead: "This policy explains what information TinyPaws uses, what is stored on your device, and what the app does not collect.", effective: "Effective date: July 28, 2026", local: "1. Information stored on your device", localLead: "TinyPaws stores a small set of preferences on your Mac so it can restore your settings the next time you open the app.", localItems: ["Selected character", "Selected size", "Selected opacity", "Selected walk speed", "Selected movement range and walking style", "A randomly generated installation identifier"], localNote: "These preferences are stored on your device using standard macOS app storage. The installation identifier is used only for the anonymous update statistics described below.", permissions: "2. Permissions", permissionsLead: "TinyPaws does not need an online account or cloud service for its core experience. Keyboard-reactive features may need macOS permissions, which are handled on your device.", logs: "3. Diagnostic logs", logsLead: "TinyPaws may write simple diagnostic messages to a temporary log file on your Mac for app status and troubleshooting.", logsNote: "These logs remain on your device unless you choose to share them with support.", analytics: "4. Anonymous update statistics", analyticsLead: "The first time TinyPaws runs after an install or update, it may send only a random installation identifier and the app version over a secure connection to count update users. We do not send or store your name, email address, IP address, device details, or usage behavior. The identifier is removed when you delete the app.", none: "5. Information TinyPaws does not collect", noneItems: ["Account registration information", "Payment information", "Analytics profiles tied to your identity", "Cloud sync data", "Browsing history", "Personal data sent to the developer during normal use"], third: "6. Third-party services", thirdLead: "TinyPaws may use Cloudflare server infrastructure to count update users. This service is not required for the core desktop-companion experience.", choices: "7. Your choices", choicesItems: ["You can review and change TinyPaws settings on this device at any time.", "You can remove the app at any time.", "You can contact support with questions about how the app works."], changes: "8. Changes", changesLead: "This policy may be updated if TinyPaws adds new features or changes data practices. Material changes will be reflected here with a revised effective date."
    }
  }
};
...
copy.ko.privacy = {
  ...copy.ko.privacy,
  effective: "시행일: 2026년 7월 31일",
  localItems: ["선택한 캐릭터와 캐릭터 수", "선택한 사이즈", "선택한 투명도", "선택한 걷기 속도", "선택한 이동 범위와 걷기 스타일"],
  localNote: "이 설정은 표준 macOS 앱 저장소를 통해 기기에 저장됩니다.",
  analytics: "4. 익명 사용 통계",
  analyticsLead: "TinyPaws는 앱 실행, 설치 또는 업데이트, 설정 변경과 도움말·업데이트 확인 같은 기능 사용을 익명으로 집계할 수 있습니다. 이 통계에는 앱 버전·빌드 번호, macOS 버전, 기기 모델·화면 해상도·언어·지역·시간대와 같은 기술 정보 및 서비스가 생성한 앱 인스턴스 식별자가 포함될 수 있지만, 이름, 이메일, 입력한 내용, 결제 정보는 전송하지 않습니다.",
  thirdLead: "익명 사용 통계에는 Google Firebase Analytics를 사용할 수 있습니다. Google은 통계 제공을 위해 기술 정보와 앱 사용 이벤트를 처리할 수 있으며, TinyPaws의 핵심 데스크톱 컴패니언 기능에 필요하지 않습니다."
};

copy.en.privacy = {
  ...copy.en.privacy,
  effective: "Effective date: July 31, 2026",
  localItems: ["Selected character and character count", "Selected size", "Selected opacity", "Selected walk speed", "Selected movement range and walking style"],
  localNote: "These preferences are stored on your device using standard macOS app storage.",
  analytics: "4. Anonymous usage statistics",
  analyticsLead: "TinyPaws may anonymously measure app launches, installs or updates, setting changes, and use of features such as help and update checks. These statistics may include technical information such as the app version and build number, macOS version, device model, screen resolution, language, region, time zone, and a service-generated app-instance identifier, but never your name, email, typed content, or payment information.",
  thirdLead: "TinyPaws may use Google Firebase Analytics for anonymous usage statistics. Google may process technical information and app-usage events to provide these statistics; this service is not required for the core desktop-companion experience."
};

const characters = [["kkeomeoksari", "Kkeomeoksari", "꺼먹살이"], ["capybara", "Capybara", "카피바라"], ["cat", "American Shorthair", "아메리칸 숏헤어"], ["russian-blue", "Russian Blue", "러시안 블루", "NEW"], ["hedgehog", "Hedgehog", "고슴도치"], ["poodle", "Poodle", "푸들"], ["hamster", "Hamster", "햄스터"], ["burger", "Burger", "버거", "SPECIAL"]];
const releaseVersions = ["1.5.4", "1.5.3", "1.5.2", "1.5.1", "1.5.0", "1.4.9", "1.4.8", "1.4.7", "1.4.6", "1.4.5", "1.4.4", "1.4.3", "1.4.2", "1.4.1", "1.4.0", "1.3.9", "1.3.8", "1.3.7", "1.3.6", "1.3.5", "1.3.4", "1.3.3", "1.3.2", "1.3.1", "1.3.0", "1.2.9", "1.2.8", "1.2.7", "1.2.6", "1.2.5", "1.2.4", "1.2.3", "1.2.2", "1.2.1", "1.2.0", "1.1.9", "1.1.8", "1.1.7", "1.1.6", "1.1.5", "1.1.4", "1.1.3", "1.1.2", "1.1.1", "1.0.9", "1.0.8", "1.0.7", "1.0.6", "1.0.5", "1.0.4", "1.0.3", "1.0.2", "1.0.1", "1.0"];
releaseVersions.unshift("1.6.10", "1.6.9", "1.6.8", "1.6.7", "1.6.6", "1.6.5", "1.6.4", "1.6.3", "1.6.2", "1.6.1", "1.6.0", "1.5.9", "1.5.8", "1.5.7", "1.5.6", "1.5.5");
...
function indexPage(c) {
  ...
  const characterCards = characters.map(([asset, en, ko, badge]) => `<div class="character${badge ? " featured" : ""}">${badge ? `<span class="character-badge">${esc(badge)}</span>` : ""}<img class="character-icon" src="./character-assets/${asset}.png" alt="" loading="lazy"><strong>${esc(c === copy.ko ? ko : en)}</strong><small>${esc(c === copy.ko ? en : ko)}</small></div>`).join("");
...
function privacyPage(c) {
  const p = c.privacy;
  return `${header(c.common)}<section class="hero"><p class="eyebrow">${esc(p.eyebrow)}</p><h1>${esc(p.heading)}</h1><p class="lead">${esc(p.lead)}</p>${nav(c.common)}</section><section class="stack"><article class="card"><p class="muted">${esc(p.effective)}<br>${esc(c.common.email)}: <a href="mailto:tinypawskr.official@gmail.com">tinypawskr.official@gmail.com</a></p></article><article class="card"><h2>${esc(p.local)}</h2><p>${esc(p.localLead)}</p>${list(p.localItems)}<p class="muted">${esc(p.localNote)}</p></article><article class="card"><h2>${esc(p.permissions)}</h2><p>${esc(p.permissionsLead)}</p></article><article class="card"><h2>${esc(p.logs)}</h2><p>${esc(p.logsLead)}</p><p class="muted">${esc(p.logsNote)}</p></article><article class="card"><h2>${esc(p.analytics)}</h2><p>${esc(p.analyticsLead)}</p></article><article class="card"><h2>${esc(p.none)}</h2>${list(p.noneItems)}</article><article class="card"><h2>${esc(p.third)}</h2><p>${esc(p.thirdLead)}</p></article><article class="card"><h2>${esc(p.choices)}</h2>${list(p.choicesItems)}</article><article class="card"><h2>${esc(p.changes)}</h2><p>${esc(p.changesLead)}</p></article></section>`;
}
...
function setupPayment() {
  ...
  const openPayment = (payment) => {
    document.querySelectorAll("[data-payment-option]").forEach((option) => { option.hidden = option.dataset.paymentOption !== payment; });
    modal.hidden = false;
    modal.querySelector("[data-close-payment]").focus();
  };
  document.querySelectorAll("[data-open-payment]").forEach((button) => button.addEventListener("click", () => openPayment(button.dataset.openPayment)));
  ...
  if (["toss", "kakaopay"].includes(paymentFromUrl)) openPayment(paymentFromUrl);
}
