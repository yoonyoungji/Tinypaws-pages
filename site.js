...
const copy = {
  ko: {
    common: { home: "홈", support: "지원", privacy: "개인정보처리방침", latest: "최신 버전 1.2.2", email: "문의 이메일", developer: "개발자: youngji Yoon", response: "답변은 보통 3영업일 안에 드립니다.", footer: "TinyPaws 공식 지원 및 개인정보처리방침", close: "닫기" },
    index: {
      title: "TinyPaws — Mac 데스크톱 펫", eyebrow: "TinyPaws · macOS 데스크톱 펫", heading: "Mac 화면에서 함께 걷는 작은 친구.", lead: "TinyPaws는 화면 위를 자연스럽게 움직이며 일상에 작은 생기를 더하는 macOS 데스크톱 펫입니다. 메뉴 막대에서 캐릭터와 움직임을 내 취향대로 설정해 보세요.", supportCta: "도움말 보기", privacyCta: "개인정보처리방침",
      ...
      release: "1.2.2 업데이트", releaseLead: "흑콩이 전용 달리기 프레임을 추가해 빠른 움직임이 더 부드러워졌고, 연기 같은 잔상 대시 효과와 간식 포즈도 다듬었습니다.",
  ...
  en: {
    common: { home: "Home", support: "Support", privacy: "Privacy Policy", latest: "Latest version 1.2.2", email: "Support email", developer: "Developer: youngji Yoon", response: "Typical response time: within 3 business days", footer: "TinyPaws official support and privacy information", close: "Close" },
    index: {
      title: "TinyPaws — A Mac desktop pet", eyebrow: "TinyPaws · macOS desktop pet", heading: "A tiny friend that walks alongside your Mac.", lead: "TinyPaws is a macOS desktop pet that moves naturally across your screen and brings a little life to everyday work. Make it yours from the menu bar.", supportCta: "Get support", privacyCta: "Privacy policy",
      ...
      release: "New in 1.2.2", releaseLead: "Heukkongi now has dedicated run frames for smoother fast movement, a smoky phantom-dash effect, and a refined snack pose.",
...
const nav = (c) => `<nav class="nav" aria-label="Page links"><a href="./index.html">${c.home}</a><a href="./support.html">${c.support}</a><a href="./privacy.html">${c.privacy}</a></nav>`;
...
function supportPage(c) {
  ...
  return `${header(c.common)}<section class="hero"><p class="eyebrow">${esc(p.eyebrow)}</p><h1>${esc(p.heading)}</h1><p class="lead">${esc(p.lead)}</p><div class="version-badge">✦ ${esc(c.common.latest)}</div>${nav(c.common)}</section><section class="grid">${card(c.common.email, `<p><a class="btn secondary" href="mailto:dbsdudwl94@gmail.com">dbsdudwl94@gmail.com</a></p><p class="muted">${esc(c.common.developer)}<br>${esc(c.common.response)}</p>`)}${card(p.overview, list(p.overviewItems))}${card(p.permissions, `<p class="muted">${esc(p.permissionsLead)}</p>`)}${card(p.compatibility, list(p.compatibilityItems))}${card(p.faq, faqs, true)}</section>`;
}

function privacyPage(c) {
  const p = c.privacy;
  return `${header(c.common)}<section class="hero"><p class="eyebrow">${esc(p.eyebrow)}</p><h1>${esc(p.heading)}</h1><p class="lead">${esc(p.lead)}</p>${nav(c.common)}</section><section class="stack"><article class="card"><p class="muted">${esc(p.effective)}<br>${esc(c.common.email)}: <a href="mailto:dbsdudwl94@gmail.com">dbsdudwl94@gmail.com</a></p></article><article class="card"><h2>${esc(p.local)}</h2><p>${esc(p.localLead)}</p>${list(p.localItems)}<p class="muted">${esc(p.localNote)}</p></article><article class="card"><h2>${esc(p.permissions)}</h2><p>${esc(p.permissionsLead)}</p></article><article class="card"><h2>${esc(p.logs)}</h2><p>${esc(p.logsLead)}</p><p class="muted">${esc(p.logsNote)}</p></article><article class="card"><h2>${esc(p.none)}</h2>${list(p.noneItems)}</article><article class="card"><h2>${esc(p.third)}</h2><p>${esc(p.thirdLead)}</p></article><article class="card"><h2>${esc(p.choices)}</h2>${list(p.choicesItems)}</article><article class="card"><h2>${esc(p.changes)}</h2><p>${esc(p.changesLead)}</p></article></section>`;
}
...
function render(locale, updateHistory = false) {
  ...
  document.title = page === "index" ? c.index.title : page === "support" ? c.support.title : c.privacy.title;
  const main = page === "index" ? indexPage(c) : page === "support" ? supportPage(c) : privacyPage(c);
